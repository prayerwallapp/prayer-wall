...
**`reactions` RLS — cross-status/cross-church SELECT leak (fixed + verified Session 19b / 2026-07-21, applied to production):** The original `reactions_public_read` policy used `USING (true)` — world-readable, no submission-status or church scoping. This allowed the anon key to read reactions on `pending`/`held` submissions (cross-status leak) and on any church's submissions (cross-church). **Fixed in `20260721_reactions_rls_and_embed_prep.sql`:** (1) dropped `reactions_public_read`; (2) dropped `reactions_public_insert` (vestigial WITH CHECK(true) anon INSERT hole from session2.sql — would have let anon bypass `insert_embed_reaction` validation); (3) created `auth_user_church_id()` SECURITY DEFINER helper (was in 002_fix_users_rls_recursion.sql but never applied to production — migration is now self-contained); (4) created `reactions_select_public` — anon and authenticated can SELECT reactions where `submission_id IN (SELECT id FROM submissions WHERE status = 'approved' AND visibility = 'public')` — mirrors `submissions_public_read_approved` (status AND visibility, not status alone); (5) created `reactions_select_own_church` — authenticated users can SELECT reactions where `church_id = public.auth_user_church_id()` (own church, any status — needed for admin moderation inbox and notification flows); (6) created `insert_embed_reaction` SECURITY DEFINER RPC for anon embed reactions — validates approved+public, embed_enabled, emoji settings before inserting with `user_id=NULL, source='embed'`. **Verified 2026-07-21 via Supabase SQL Editor (8 checks):** cross-status leak closed ✅, approved+public reads intact ✅, approved+private scoping correct ✅, authenticated own-church reads work ✅, embed RPC happy path works ✅, RPC rejects non-approved/embed-disabled/invalid-emoji ✅, direct anon INSERT blocked by RLS ✅, realtime publication intact ✅. **Residual exposure (accepted, low-severity):** `user_id` (reactor UUID) is still visible on approved+public submissions' reactions to any anon caller. `users` table RLS prevents resolving a UUID to identity. This was the original accepted MVP risk — it remains accepted post-fix, and is now narrower (approved+public only). **Judgment call — church_id scoping of approved reads:** scopes by status+visibility only, not church_id. Anon has no session-based church context; approved+public reactions are public data by definition. Consistent with `submissions_public_read_approved`.
...
- [x] Supabase auth (email + Google SSO) — Google OAuth tested end-to-end **against local/dev (Session 8)**: redirect → callback → users row created with correct church_id, display_name pre-populated from Google metadata, re-sign-in matches existing row (no duplicate)
  - **Production OAuth config verification (BUILD-15 / 2026-07-21) — CONFIRMED against real prod (`prayerwallapp.com`, Supabase `vugttmpvqvqvkktlebmf`):** prod domain live (HTTP 200); Google provider **enabled** in prod Supabase; prod OAuth **client_id `633255818234-seis2ogp0bi7e6q59tm3bn34d01cfj50.apps.googleusercontent.com`**; Google Console **has the Supabase callback authorized** — `https://vugttmpvqvqvkktlebmf.supabase.co/auth/v1/callback` (probe returned **no `redirect_uri_mismatch`**); consent screen **published** (no `access_blocked`); a real system-Chrome session driven against the prod authorize URL **reached Google's genuine "Sign in - Google Accounts" page**. App code builds `redirectTo` dynamically from `window.location.origin` (no hardcoded localhost — `SubmissionModal.tsx:165`) and the callback prefers the `x-church-subdomain` header to survive Supabase's redirect_to allow-list substituting Site URL (`app/auth/callback/route.ts:29-30`).
  - **CONFIRMED LIVE by Josiah (2026-07-21) — "never confirmed live" gap now CLOSED:** (1) the completed production round-trip — Google sign-in → session established → `users` row — works end-to-end in prod (manual human verification, the step Claude Code cannot perform autonomously). (2) Supabase Auth **Site URL** and redirect settings are set correctly for production. Google OAuth is now confirmed working live in production.
